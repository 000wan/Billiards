        ...
        for j in range(i+1,len(balls)):
            ...
            if (b1.x-b2.x)**2+(b1.y-b2.y)**2<=4*R**2:
                if not col[name][0]:
                    ...
                    #col[name][2]=[b1,b2] if b1.v>b2.v else [b2,b1] # victim
                    ...
                    '''(b1.vx, b1.vy)=(-b2.v*cos(col[name][1])*cos(b2.deg-col[name][1])+b1.v*sin(col[name][1])*sin(b1.deg-col[name][1]),
                                    -b2.v*sin(col[name][1])*cos(b2.deg-col[name][1])-b1.v*cos(col[name][1])*sin(b1.deg-col[name][1]))
                    (b2.vx, b2.vy)=( b1.v*cos(col[name][1])*sin(b1.deg-col[name][1])-b2.v*sin(col[name][1])*sin(b2.deg-col[name][1]),
                                     b1.v*sin(col[name][1])*cos(b1.deg-col[name][1])+b2.v*cos(col[name][1])*sin(b2.deg-col[name][1]))'''
                '''elif (b1.x-b2.x)**2+(b1.y-b2.y)**2<=4*(R-2*dl)**2:
                    #exception
                    b1.x,b1.y=b1.x-cos(col[name][1]), b1.y-sin(col[name][1])
                    b2.x,b2.y=b2.x+cos(col[name][1]), b2.y+sin(col[name][1])'''
            else:
                col[name][0]=False
...
def turnOver(balls,p1,p2):
    ...
    #minus points
    if havcol['ybwb'] or any([havcol[nturn.name+'rb1'],havcol[nturn.name+'rb2']]) or all([not havcol[turn.name+'rb1'], not havcol[turn.name+'rb2']]):
        score[balls.index(turn)]-=10
        turn,nturn=nturn,turn
    #plus points
    elif havcol['rb1rb2'] or all([havcol[nturn.name+'rb1'],havcol[nturn.name+'rb2']]):
        score[balls.index(turn)]+=10
    #no points
    else:
        turn,nturn=nturn,turn
...
def main():
    ...
    while in_main:
        ...
        if p1_name.get_text()=='' or p1_name.get_text()=='':
